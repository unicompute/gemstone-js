...
- `Session.dictionaryOopToObject()` and `dictionaryValues()` read GemStone
  ...
  Session-level dictionary key/item/value-list and size helpers delegate to the
  same wrapper path for callers that have a raw dictionary OOP but do not need
  to keep a `GsDict` wrapper around.
