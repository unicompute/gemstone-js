...
test("arrayOopToValues converts GemStone Arrays back to JavaScript values", async () => {
  ...
  assertEqual(await session.arrayFirstValue(array), "Ada");
  assertEqual(await session.arrayFirst(array), "Ada");
  assertEqual(await session.arrayFirstOop(array), name);
  const firstObject = await session.arrayFirstObject(array);
  assertEqual(firstObject?.oop, name);
  await firstObject?.release();
  assertEqual(await session.arrayLastOop(array), nested);
  ...
  assertEqual(await session.arrayFirstOop(emptyArray), null);
  assertEqual(await session.arrayFirstObject(emptyArray), null);
...
test("dictionaryToOop stores and retrieves string-key values", async () => {
  ...
  const pickedOops = await session.dictionaryPickOop(dict, ["object", "missing"]);
  assertEqual(pickedOops.object, object);
  assertEqual(pickedOops.missing, null);
  const pickedObjects = await session.dictionaryPickObject<{ name: string }>(dict, ["object", "missing"]);
  assertEqual(pickedObjects.object?.oop, object);
  assertEqual(pickedObjects.missing, null);
  await pickedObjects.object?.release();
  assertEqual(await (await session.dictionaryPickDict(dict, ["nested", "missing"])).nested?.get("status"), "child");
  assertEqual(await session.dictionaryRequireValue(dict, "name"), "Alice");
  assertEqual(await session.dictionaryRequireOop(dict, "object"), object);
  assertDeepEqual(await session.dictionaryRequireAllValue(dict, ["name"]), { name: "Alice" });
  const requiredOops = await session.dictionaryRequireAllOop(dict, ["object"]);
  assertEqual(requiredOops.object, object);
  const requiredObject = await session.dictionaryRequireObject<{ name: string }>(dict, "object");
  assertEqual(requiredObject.oop, object);
  await requiredObject.release();
  const requiredAlias = await session.dictionaryRequire<{ name: string }>(dict, "object");
  assertEqual(requiredAlias.oop, object);
  await requiredAlias.release();
  const requiredObjects = await session.dictionaryRequireAllObject<{ name: string }>(dict, ["object"]);
  assertEqual(requiredObjects.object.oop, object);
  await requiredObjects.object.release();
  const requiredAliases = await session.dictionaryRequireAll<{ name: string }>(dict, ["object"]);
  assertEqual(requiredAliases.object.oop, object);
  await requiredAliases.object.release();
  assertEqual(await (await session.dictionaryRequireDict(dict, "nested")).get("status"), "child");
