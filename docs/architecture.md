...
- Query helpers render simple selector paths, expose collection metadata,
  ...
  read whole collections, bounded pages, or collection endpoints as handles, can
  check, add, remove, replace, or clear collection members through explicit
  value and raw-OOP helpers, and unwrap GemStone arrays through `size`/`at:` so
  iterators yield object handles instead of chunk containers.
